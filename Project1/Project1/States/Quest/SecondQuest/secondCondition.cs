...
{
    internal class secondCondition : State
    {
        ...
        {
            "Но это все потом, надо сейчас решить, как мне быть, ведь \n Поговаривают, что в этом лесу водятся волки и медведи.",
            "Но я не особо в это верю, ведь лес находится прям рядом с городом.",
            "Тем более это основное место для добычи мясных припасов в нашем городишке.",
            "Но может все-таки стоит перестраховаться?",
        };
        ...
        public secondCondition(Game1 game1, GraphicsDevice graphicsDevice, ContentManager content, StateInfo stateInfo = null) : base(game1, graphicsDevice, content)
        {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (keyboardState.IsKeyDown(Keys.Escape))
            {
                ...
                _game.ChangeState(new secondConditionEsc(_game, _graphicsDevice, _content, currentStateInfo));
                return;
            }
            ...
            if (mouseState.LeftButton == ButtonState.Pressed && previousMouseState.LeftButton == ButtonState.Released)
            {
                ...
                if (_textIndex == 4)
                {
                    _game.ChangeState(new secondQuest(_game, _graphicsDevice, _content));
                }
            }
