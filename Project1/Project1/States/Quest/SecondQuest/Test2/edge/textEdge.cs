...
namespace Project1.States.Quest.SecondQuest.Test2.edge
{
    internal class textEdge : State
    {
        ...
        {
            "Я все-таки решил пойти рядом с лесом, с надеждой обогнуть его,",
            "Это получится чуть дольше, но меньше вероятность встретить каких-то диких животных.",
            "Спустя часа 2 я случайно вышел на лагерь каких-то людей.",
            "Как оказалось, это был лагерь каких-то бандитов, \n которые решили напасть наменя..."
        };
        ...
        public textEdge(Game1 game1, GraphicsDevice graphicsDevice, ContentManager content, StateInfo stateInfo = null) : base(game1, graphicsDevice, content)
        {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (mouseState.LeftButton == ButtonState.Pressed && previousMouseState.LeftButton == ButtonState.Released)
            {
                ...
                if (_textIndex == 4)
                {
                    _game.ChangeState(new edgeUrealFight(_game, _graphicsDevice, _content));
                }
            }
