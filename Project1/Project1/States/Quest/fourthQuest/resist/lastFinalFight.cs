...
namespace Project1.States.Quest.fourthQuest.resist
{
    public class lastFinalFight : State
    {
        ...
        private float _characterScale = 0.6f;
        ...
        private string _regenText = "";

        public lastFinalFight(Game1 game1, GraphicsDevice graphicsDevice, ContentManager content) : base(game1, graphicsDevice, content)
        {
            ...
            _imageTexture = _content.Load<Texture2D>("ggEnemy/boss");
            ...
            var regenButton = new Button(buttonTexture, buttonFont)
            {
                Position = new Vector2(1350, 800),
            ...
            _components = new List<Component>()
            {
                ...
                new ImageComponent(_imageTexture, new Vector2(1100, 0), _characterScale),
                attackButton, deffButton, archerButton, regenButton
            };
        }

        private async void regenButton_Click(object sender, EventArgs e)
        {
            Random rnd = new Random();
            _enemyDamage = rnd.Next(150, 160);
            ...
            if (_heroHP < 500)
            {
                _heroHP = Math.Min(_heroHP + _heroRegen + 200 - _heroDamageTaken, 500);
            }

            if (_heroRegen < 0)
                _regenText = $"Вы восстановили {_heroRegen} единиц здоровья";
            else
                _regenText = $"Вы восстановили {_heroRegen + 200} единиц здоровья";
        }
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            Vector2 regenPosition = new Vector2(50, 650);
            spriteBatch.DrawString(_font, _regenText, regenPosition, Color.Green);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (_enemyHP <= 0)
            {
                _game.ChangeState(new secondFinal(_game, _graphicsDevice, _content));
            }

            if (_heroHP <= 0)
            {
                _game.ChangeState(new thirdFinal(_game, _graphicsDevice, _content));
            }

            _characterHealthText = $"Ваше здоровье: {_heroHP}";
            _enemyHealthText = $"Здоровье Холдена: {_enemyHP}";
        }
    }
}
