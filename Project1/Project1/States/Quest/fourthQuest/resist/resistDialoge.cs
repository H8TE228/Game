...
internal class resistDialoge : State
{
    ...
    {
        "Ну что, это твой выбор, а теперь позволь мне показать тебе свое истинное обличие.",
        "Ну как, ты впечатлен?",
        "Как ты уже начал догадываться я - воплощение самого зла.",
        "Меня все зовут судья.",
        "Судья Холден.",
        "А Саркон это так, прозвище.",
        "Только я решаю жить кому-либо или умереть.",
        "Так вот, ты",
        "УМРЕШЬ!!!"
    };
    ...
    public resistDialoge(Game1 game1, GraphicsDevice graphicsDevice, ContentManager content, StateInfo stateInfo = null) : base(game1, graphicsDevice, content)
    {
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (mouseState.LeftButton == ButtonState.Pressed && previousMouseState.LeftButton == ButtonState.Released)
        {
            ...
            if (_textIndex == 9)
            {
                _game.ChangeState(new lastFinalFight(_game, _graphicsDevice, _content));
            }
        }
