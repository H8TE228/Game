...
{
    internal class mazeFirst : State
    {
        ...
        public mazeFirst(Game1 game1, GraphicsDevice graphicsDevice, ContentManager content) : base(game1, graphicsDevice, content)
        {
            ...
            var skipButton = new Button(buttonTexture, buttonFont)
            {
                Position = new Vector2(625, 600),
                Text = "Пропустить лабиринт",
                Scale = 0.75f
            };
        ...
        {
            _game.ChangeState(new mazeSecond(_game, _graphicsDevice, _content));
        }

        private void rightGameButton_Click(object sender, EventArgs e)
        {
            _game.ChangeState(new mazeThird(_game, _graphicsDevice, _content));
        }
